...
class UpdateAssetRequest extends FormRequest
{
    ...
    public function rules(): array
    {
        ...
        return [
            ...
            'location_id' => 'nullable|exists:location,id', // Optional but must reference an existing location
            'manufacturer_id' => 'nullable|exists:manufacturer,id', // Optional but must reference an existing manufacturer
            'assigned_to_user_id' => 'nullable|exists:users,id', // Optional but must reference an existing user
            'asset_tag' => 'nullable|string|max:255|unique:asset,asset_tag,' . $asset->id, // Optional but must be unique, ignoring the current asset
            'name' => 'required|string|max:255|unique:asset,name,' . $asset->id, // Required, unique, max 255 characters, ignoring the current asset
            'serial_number' => 'required|string|max:255|unique:asset,serial_number,' . $asset->id, // Required, unique, max 255 characters, ignoring the current asset
